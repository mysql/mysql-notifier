...
{
  public partial class NewNonWindowsConnectionDialog : LoginGathererForm
  {
    private MySqlWorkbenchConnection WBconn;

    public NewNonWindowsConnectionDialog()
    {
      InitializeComponent();
    ...
    private void okButton_Click(object sender, EventArgs e)
    {
      ...
      if (savePasswordInVault.Checked)
      {
        Login = new AccountLogin(WBconn.Host, WBconn.Port, WBconn.UserName, WBconn.Password);
      }
      else
      {
        Login = new AccountLogin(WBconn.Host, WBconn.Port, WBconn.UserName, string.Empty);
      }
    }
    ...
    private void testButton_Click(object sender, EventArgs e)
    {
      ...
      //InfoDialog infoDialog = new InfoDialog(false, String.Format(Resources.ConnectionDataDisplayFailed, testHostName, testConn.Port, testUserName), string.Empty);

      //try
      //{
      //  if (!TryOpenConnection(testConn)) return;
      //  infoDialog = new InfoDialog(true, String.Format(Resources.ConnectionDataDisplaySuccess, testHostName, testConn.Port, testUserName), string.Empty);
      //}
      //catch (Exception ex)
      //{
